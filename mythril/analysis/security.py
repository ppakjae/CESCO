...
from mythril.analysis.module import ModuleLoader, reset_callback_modules, reset_compile_modules
...
def retrieve_compile_issues(white_list: Optional[List[str]] = None) -> List[Issue]:
    """Get the issues discovered by compile type detection modules"""
    issues = []  # type: List[Issue]
    for module in ModuleLoader().get_detection_modules(
        entry_point=EntryPoint.COMPILE, white_list=white_list
    ):
        log.debug("Retrieving results for " + module.name)
        issues += module.issues

    reset_compile_modules(module_names=white_list)
...
def fire_lasers(statespace, white_list: Optional[List[str]] = None) -> List[Issue]:
    ...
    issues += retrieve_compile_issues(white_list)
